...
Object* initTestInventory5(){
    Object* inventory = new_Object();

    inventory->type = ARME_TYPE;
    inventory->arme = getDBArme(DIAMOND_HAMMER);
    inventory->isSelected = SELECTED;
    ...
    inventory->next->next = new_Object();
    inventory->next->next->type = ARMURE_TYPE;
    inventory->next->next->armure = getDBArmure(IRON_CHESTPLATE);
...
void testUtilsFuntions(){
    ...
    Object* isCopiedObject = objectMemCpy(searchObjectByType(inventory, RDC_TYPE));
    ...
    printf("--Test objectMemCpy object function...                          Result: %d\n",isCopiedObject!=NULL);
    printf("\n");
...
void testChestFunctions(){
    printf("//---------// Chest Functions //---------//\n");
    ...
    Object** tmp = addObjectToChest(inventory, chest, LAVENDER);
    chest = tmp[0];
    inventory = tmp[1];
    int inventoryToChestCondition = searchObjectById(chest, LAVENDER)!=NULL && searchObjectById(inventory, LAVENDER)==NULL;
    Object** tmp2 = getObjectFromChest(chest, inventory, LAVENDER);
    inventory = tmp2[0];
    chest = tmp2[1];
    
    int chestToInventoryCondition = searchObjectById(chest, LAVENDER)==NULL && searchObjectById(inventory, LAVENDER)!=NULL;
    // printf("### searchObjectById(chest, LAVENDER)==NULL %d and searchObjectById(inventory, LAVENDER)!=NULL %d\n",searchObjectById(chest, LAVENDER)==NULL,searchObjectById(inventory, LAVENDER)!=NULL);

    finishedTreatmentLog();

    printf("--Test addObjectToChest function...                             Result: %d\n",inventoryToChestCondition);
    printf("--Test getObjectFromChest function...                           Result: %d\n",chestToInventoryCondition);
    printf("\n\n");
}
...
void testUpdateWeaponOrToolFunctions(){
    ...
    printf("\n\n*****TO DO: Please enter 'Y' when we will ask for the craft test, Thanks!*****\n\n");
...
void testCraftRecipesDataBaseFunctions(){
    ...
    int* craftRecipe = craftObjectRecipe(DIAMOND_HAMMER);
...
void testAttacksFunctions(){
    printf("//---------// Attacks player-monster //---------// \n");

    startTreatmentLog();

    Player* player = initTestPlayer1();
    player->inventory = initTestInventory5();
    Monster* monster = initTestMonter();

    int isDead = attackMonster(player, monster);
    int attackMonsterCondition1 = monster->pv==85 && isDead==1;
    isDead = attackMonster(player, monster);
    isDead = attackMonster(player, monster);
    isDead = attackMonster(player, monster);
    isDead = attackMonster(player, monster);
    isDead = attackMonster(player, monster);
    isDead = attackMonster(player, monster);
    int attackMonsterCondition2 =  monster->pv==0 && isDead==0 && player->xp==monster->xp_profit;
    int attackMonsterCondition = attackMonsterCondition1 && attackMonsterCondition2;

    isDead = receiveDamage(player, monster); 
    int receiveDamageCondition1 = player->hp_current==76 && isDead==1;
    isDead = receiveDamage(player, monster);  
    isDead = receiveDamage(player, monster);  
    isDead = receiveDamage(player, monster);  
    isDead = receiveDamage(player, monster);  
    int receiveDamageCondition2 = player->hp_current==0 && isDead==0;
    int receiveDamageCondition = receiveDamageCondition1 && receiveDamageCondition2;

    printf("--Test attackMonster function...                                Result: %d\n",attackMonsterCondition);
    printf("--Test receiveDamage function...                                Result: %d\n",receiveDamageCondition);
