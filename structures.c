...
/*
Objects* newObject(int type, int valeur){
    Objects* object = malloc(sizeof(Objects));

    if ( type == ARME_TYPE ){
        addarme(object, valeur);
    }
    else if ( type == RDC_TYPE ){
        addcraft(object, valeur);
    }
    else if ( type == ARMURE_TYPE || type == OUTIL_TYPE || type == SOIN_TYPE ){
        addobject(object, valeur);
    }
    return object;
}*/
